...
import { setAllMessages, setAllUsers, setChatId } from "../redux/slices/chatSlice";
...
export const postLikeUnlike = async (postId, currUserId, currUser) => {
    try {
        ...
        if (post.likes.includes(currUserId)) {
            ...
            const res = await addNotification(currUserId, post.ownerId, "Liked your post", post.title, post.postUrl, currUser?.photoUrl, currUser?.name)
            if (res.status == 'error') {
                alert(res.message);
            }
            else {
...
export const userFollowUnFollow = async (id, currUserId) => {
    ...
    if (currUser.followings.includes(id)) {
        ...
        const res = await addNotification(currUserId, id, "Unfollowed You", '', '', currUser.photoUrl, currUser.name)
        ...
        const res = await addNotification(currUserId, id, "Followed You", '', '', currUser.photoUrl, currUser.name)
...
export const addComment = async (post, currUserId, msg, createdAt, currUser) => {
    try {
        ...
        const res = await addNotification(currUserId, post.ownerId, "Commented on your post", post.title, post.postUrl, currUser.photoUrl, currUser.name)
...
export const getAllUsers = async (email) => {
    try {
        ...
        const querySnapshot = await getDocs(q);
        store.dispatch(setAllUsers(querySnapshot.docs.map((doc) => ({ ...doc.data(), id: doc.id }))));
...
export const getChatId = async (chatUserId, currUserId) => {
    try {
        ...
        let conversationCollectionRef = collection(db, `chats/${chatId}/conversations`);
        let docSnap = await getDocs(conversationCollectionRef);
        if (!docSnap.empty) {
            console.log('1');
            store.dispatch(setChatId(chatId));
            store.dispatch(setAllMessages(docSnap.docs.map((doc) => ({ ...doc.data(), id: doc.id }))));
            return;
        }
        else {
            chatId = `${currUserId}-${chatUserId}`;
            conversationCollectionRef = collection(db, `chats/${chatId}/conversations`);
...
export const addMessage = async (chatId, chatUserId, currUserId, text, file) => {
    try {
        ...
        if (chatId) {
            ...
            conversationCollectionRef = collection(db, `chats/${newChatId}/conversations`);
        }
...
export const deleteMessage = async (msg, chatId) => {
    try {
        if (!msg.fileType) {
            ...
            deleteObject(msgFileRef).then(async () => {
                const msgDoc = doc(db, `chats/${chatId}/conversations/`, msg.id);
                await deleteDoc(msgDoc);
                return success('message deleted');
            }).catch((e) => {
                console.log(e);
                return error('This is a premium feature, to use this plz pay 1cr to developer');
            });
        }
    } catch (e) {
        console.log(e);
...
export const addNotification = async (senderId, recieverId, message, postTitle, postUrl, senderPhotoUrl, senderName) => {
    try {
        const allNotifications = store.getState().notificationReducer.allNotifications;
        ...
        if (allNotifications.length >= 10) {
            const lastNotificationDoc = doc(db, `users/${recieverId}/Notifications`, allNotifications[allNotifications.length - 1].id);
            await deleteDoc(lastNotificationDoc);
        }

        const notificationCollectionRef = collection(db, `users/${recieverId}/Notifications`);
...
export const getNotifications = (userId) => {
    try {
        ...
        const q = query(collection(db, `users/${userId}/Notifications`), orderBy('createdAt'));
        const unsub = onSnapshot(q, (querySnapshot) => {
            ...
            querySnapshot.forEach(async (docSnap) => {
                ...
                if (Date.now() - newNotification.createdAt >= 86400000) {
                    ...
                    const notificationDoc = doc(db, `users/${userId}/Notifications`, newNotification.id);
